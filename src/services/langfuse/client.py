...
class LangfuseTracer:
    ...
    def trace_rag_request(
        ...
        """
        Compatibility wrapper for the existing RAGTracer.

        Langfuse v3 uses CallbackHandler/start_span instead of the
        older trace_rag_request API.
        """
        if not self.client:
            yield None
            return
    ...
    def create_span(
        ...
        Maps the old create_span API to Langfuse v3 start_span.
        """
        if not self.client:
            return None
