...
async def ainvoke_rewrite_query_step(
    ...
    try:
        ...
        logger.debug(
            f"Invoking LLM for query rewriting "
            f"(model: {runtime.context.model_name})"
        )
        llm_start = time.time()

        # Generate structured JSON directly through Ollama
        ...
        response = QueryRewriteOutput(**response_data)

        if not response.rewritten_query:
            raise ValueError(
                "LLM failed to return valid rewritten query"
            )
        ...
        logger.info(
            ...
            f"'{original_question[:50]}...' -> "
            f"'{rewritten_query[:50]}...'"
        )
