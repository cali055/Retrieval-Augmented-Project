...
async def ainvoke_guardrail_step(
    ...
    try:
        ...
        # Invoke Ollama directly through the existing OllamaClient.
        # We don't use LangChain here because OllamaClient doesn't expose
        # get_langchain_model().
        logger.info("Invoking LLM for guardrail validation")

        result = await runtime.context.ollama_client.generate(
            model=runtime.context.model_name,
        ...
        response = GuardrailScoring(**response_data)

        logger.info(
            f"Guardrail result - Score: {response.score}, "
            f"Reason: {response.reason}"
        )

        # Update Langfuse span with successful result
        if span:
        ...
        logger.error(
            f"LLM guardrail validation failed: {e}, "
            "falling back to default"
        )

        # Conservative fallback if the LLM call or JSON parsing fails
